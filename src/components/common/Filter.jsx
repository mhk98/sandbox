import Collapse from 'components/reuseable/Collapse'; // -------- data -------- //
...
const Filter = () => {
  const Title = title => <h4 className="widget-title mb-3">{title}</h4>;

  return <aside className="col-lg-3 sidebar">
      <div className="widget mt-1">
        <h4 className="widget-title mb-3">Categories</h4>
        <ul className="list-unstyled ps-0">
          {collapseList.map(item => <li className="mb-1" key={item.id}>
              <Collapse {...item} />
            </li>)}
        </ul>
      </div>
